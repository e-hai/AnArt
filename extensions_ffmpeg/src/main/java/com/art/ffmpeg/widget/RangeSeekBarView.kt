...
class RangeSeekBarView(context: Context, minRangeTime: Long, maxRangeTime: Long) : View(context) {
    ...
    init {
        ...
        val shadowColor = context.resources.getColor(R.color.ffmpeg_shadow_color)

        //初始化指示器
        thumbLeftBitmap = getScaleBitmap(
            BitmapFactory.decodeResource(resources, R.drawable.ffmpeg_icon_thumb_left),
            thumbWidth,
            thumbHeight
        )
        thumbRightBitmap = getScaleBitmap(
            BitmapFactory.decodeResource(resources, R.drawable.ffmpeg_icon_thumb_right),
            thumbWidth,
            thumbHeight
        )
